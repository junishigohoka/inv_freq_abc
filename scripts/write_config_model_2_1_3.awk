...
func make_log_line(N_anc, Ns_N0, T_geo, T, pops, simid, seed,    pop, str){
        ...
        str = str " " T_geo " " T["short"] " " T["res"] " " T["mac1"] " " T["mac2"] " " T["azo_cap"] " " T["can_mad"] " " T["cre"] " " T["mal"]
        return str
}
...
func make_log_header(pops,    pop, str){
        ...
        str=str "T_geo T_short T_res T_mac1 T_mac2 T_azo_cap T_can_mad T_cre T_mal"
        return str
}
...
END{
        ...
        # Get T_split
        T["short"] = logunif(1e4, 1e5)
        T["res"] = logunif(1e4, 1e5)
        T["mac1"] = logunif(1e4, 1e5)
        T["mac2"] = logunif(1e4, 1e5)
        T["azo_cap"] = logunif(1e4, 1e5)
        T["can_mad"] = logunif(1e4, 1e5)
